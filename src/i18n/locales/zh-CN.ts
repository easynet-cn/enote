export default {
  // 通用
  common: {
    save: '保存',
    cancel: '取消',
    delete: '删除',
    edit: '编辑',
    confirm: '确认',
    search: '搜索',
    name: '名称',
    description: '描述',
    sort: '排序',
    icon: '图标',
    create: '创建',
    add: '添加',
    loading: '加载中...',
    empty: '暂无数据',
    required: '必填',
    yes: '是',
    no: '否',
    close: '关闭',
    clear: '清除',
    retry: '重试',
    errorBoundaryMessage: '组件出现异常，请重试',
    apply: '应用',
    reset: '重置',
    export: '导出',
    import: '导入',
    settings: '设置',
    more: '更多',
    pleaseSelect: '请选择',
    noMatchingOptions: '没有匹配的选项',
    confirmAction: '确认操作',
  },

  // 数据备份
  backup: {
    title: '数据备份',
    tabExport: '导出备份',
    tabImport: '导入恢复',
    selectFormat: '选择备份格式',
    sqlDesc: '导出为标准 SQL 语句，适合跨数据库迁移',
    excelDesc: '导出为 Excel 表格，每张表一个 Sheet',
    csvDesc: '导出为 CSV 文件，打包为 ZIP 压缩包',
    exportButton: '导出',
    importButton: '导入',
    exportSuccess: '数据备份导出成功',
    exportError: '数据备份导出失败',
    importSuccess: '数据恢复导入成功，请刷新页面',
    importError: '数据恢复导入失败',
    importWarning: '导入操作将覆盖当前所有数据，请确保已备份当前数据。此操作不可撤销。',
    importConfirm: {
      title: '确认导入',
      message: '导入将清除当前所有数据并替换为备份数据，此操作不可恢复。确定要继续吗？',
      confirmText: '确认导入',
    },
  },

  // 侧边栏
  sidebar: {
    collapse: '收起侧边栏',
    expand: '展开侧边栏',
    notebooks: '笔记本',
    tags: '标签',
    createNotebook: '添加笔记本',
    dataBackup: '数据备份',
    editNotebook: '编辑笔记本',
    deleteNotebook: '删除笔记本',
    createTag: '添加标签',
    editTag: '编辑标签',
    deleteTag: '删除标签',
    importNotes: '导入笔记',
    selectIcon: '选择图标',
    selectColor: '选择颜色样式',
    notesCount: '笔记数量',
    notebookForm: {
      title: '笔记本表单',
      nameLabel: '名称',
      namePlaceholder: '请输入笔记本名称',
      descriptionLabel: '描述',
      descriptionPlaceholder: '请输入描述',
      sortOrderLabel: '排序',
      sortOrderPlaceholder: '请输入排序序号',
      parentLabel: '上级笔记本',
      noParent: '无（顶级笔记本）',
      createTitle: '添加笔记本',
      editTitle: '编辑笔记本',
    },
    tagForm: {
      title: '标签表单',
      nameLabel: '名称',
      namePlaceholder: '请输入标签名称',
      iconLabel: '图标',
      iconPlaceholder: '选择图标',
      colorLabel: '样式颜色',
      colorPlaceholder: '选择颜色样式',
      sortOrderLabel: '排序',
      sortOrderPlaceholder: '请输入排序序号',
      createTitle: '添加标签',
      editTitle: '编辑标签',
    },
    deleteNotebookConfirm: {
      title: '删除笔记本',
      message: '确定要删除这个笔记本吗？笔记本下的所有笔记也将被删除，此操作不可恢复。',
      confirmText: '删除',
    },
    deleteTagConfirm: {
      title: '删除标签',
      message: '确定要删除这个标签吗？此操作不可恢复。',
      confirmText: '删除',
    },
    recentEdits: '最近编辑',
  },

  // 笔记本
  notebook: {
    all: '全部',
  },

  // 标签
  tag: {
    all: '全部',
  },

  // 笔记列表
  noteList: {
    searchPlaceholder: '搜索笔记...',
    clearSearch: '清除搜索',
    collapseList: '收起列表',
    expandList: '展开列表',
    empty: '还没有笔记，点击创建新笔记',
    noResults: '没有找到匹配的笔记',
    noTitle: '无标题',
    noContent: '无内容',
    updateTime: '更新时间',
    sortBy: '排序',
    sortUpdateTime: '更新时间',
    sortCreateTime: '创建时间',
    sortTitle: '标题',
    sortAsc: '升序',
    sortDesc: '降序',
    selectMode: '多选',
    selectedCount: '已选 {count} 项',
    selectAll: '全选',
    batchMove: '移动到...',
    batchDelete: '批量删除',
    batchDeleteConfirm: '确定要删除选中的 {count} 条笔记吗？',
    cancelSelect: '取消',
  },

  // 编辑器
  editor: {
    toggleEditMode: '编辑模式',
    toggleViewMode: '查看模式',
    newNote: '新建笔记',
    saveNote: '保存笔记',
    cancelEdit: '取消编辑',
    deleteNote: '删除笔记',
    noteSettings: '笔记设置',
    historyButton: '历史记录',
    wordCount: '字数统计',
    export: '导出',
    noteTitle: '笔记标题',
    noteTitlePlaceholder: '请输入笔记标题',
    noteContentPlaceholder: '开始输入内容...',
    editorPlaceholder: '在此输入笔记内容...',
    selectNotebook: '选择笔记本',
    selectTags: '选择标签',
    noTags: '暂无标签',
    noNoteSelected: '从左侧选择一篇笔记开始编辑',
    deleteNoteConfirm: {
      title: '删除笔记',
      message: '确定要删除这条笔记吗？此操作不可恢复。',
      confirmText: '删除',
    },
    unsavedChanges: {
      title: '未保存的更改',
      message: '您有未保存的更改，确定要离开吗？',
      confirmText: '离开',
      cancelText: '取消',
    },
    toolbar: {
      heading: '标题',
      paragraph: '段落',
      bold: '加粗',
      italic: '斜体',
      underline: '下划线',
      strike: '删除线',
      code: '行内代码',
      codeBlock: '代码块',
      link: '链接',
      image: '图片',
      table: '表格',
      bulletList: '无序列表',
      orderedList: '有序列表',
      taskList: '任务列表',
      quote: '引用',
      divider: '分割线',
      textAlignLeft: '左对齐',
      textAlignCenter: '居中对齐',
      textAlignRight: '右对齐',
      highlight: '高亮',
      textColor: '文字颜色',
      bgColor: '背景颜色',
      fontFamily: '字体',
      fontSize: '字号',
      superscript: '上标',
      subscript: '下标',
      undo: '撤销',
      redo: '重做',
    },
    toolbarTooltip: {
      contentType: '内容格式（保存后不可更改）',
      headingLevel: '标题级别',
      fontFamily: '字体',
      fontSize: '字号',
      bold: '粗体',
      italic: '斜体',
      underline: '下划线',
      strikethrough: '删除线',
      superscript: '上标',
      subscript: '下标',
      alignLeft: '左对齐',
      alignCenter: '居中',
      alignRight: '右对齐',
      alignJustify: '两端对齐',
      bulletList: '无序列表',
      orderedList: '有序列表',
      taskList: '任务列表',
      outdent: '减少缩进',
      indent: '增加缩进',
      quote: '引用',
      codeBlock: '代码块',
      inlineCode: '行内代码',
      codeLanguage: '代码语言',
      plainText: '纯文本',
      link: '链接',
      unlink: '取消链接',
      image: '图片',
      insertTable: '插入表格',
      addColumn: '添加列',
      addRow: '添加行',
      deleteTable: '删除表格',
      highlight: '高亮',
      divider: '分隔线',
      clearFormat: '清除格式',
      findReplace: '查找替换',
      toc: '目录',
      undo: '撤销',
      redo: '重做',
      textColor: '文本颜色',
      backgroundColor: '背景颜色',
      sourceMode: '预览模式',
      markdownSource: 'Markdown 源码',
      verticalLayout: '上下布局',
      horizontalLayout: '左右布局',
      edit: '编辑',
      save: '保存',
      cancel: '取消',
      settings: '设置',
      export: '导出',
      history: '历史记录',
      delete: '删除',
      saveAsTemplate: '存为模板',
      print: '打印',
      scrollLeft: '向左滚动',
      scrollRight: '向右滚动',
    },
    editorDialog: {
      title: '笔记标题',
      content: '在此输入笔记内容...',
      markdownSource: '在此输入 Markdown 源码...',
    },
    markdownEditor: {
      source: '源码',
      preview: '预览',
      sourcePlaceholder: '在此输入 Markdown 源码...',
    },
    linkDialog: {
      title: '插入链接',
      urlLabel: '链接地址',
      url: 'https://example.com',
      imageUrlLabel: '图片地址',
      imageUrl: 'https://example.com/image.png',
      findPlaceholder: '输入要查找的文本',
      replacePlaceholder: '输入替换文本',
      findLabel: '查找',
      replaceLabel: '替换为',
      caseSensitive: '区分大小写',
      replace: '替换',
      replaceAll: '全部替换',
      noResults: '无结果',
      or: '- 或 -',
      uploadImage: '上传图片',
    },
    searchDialog: {
      title: '查找替换',
      findLabel: '查找',
      replaceLabel: '替换为',
      caseSensitive: '区分大小写',
      replace: '替换',
      replaceAll: '全部替换',
      noResults: '无结果',
      findPrevious: '上一个',
      findNext: '下一个',
    },
    fontOptions: {
      default: '默认字体',
      sansSerif: '无衬线字体',
      serif: '衬线字体',
      monospace: '等宽字体',
      artistic: '艺术字体',
      defaultSize: '默认',
    },
    headingOptions: {
      normal: '正文',
      h1: '标题 1',
      h2: '标题 2',
      h3: '标题 3',
      h4: '标题 4',
      h5: '标题 5',
      h6: '标题 6',
    },
    contentType: {
      richText: '富文本',
      markdown: 'Markdown',
    },
    history: {
      title: '历史记录',
      empty: '暂无历史记录',
      version: '版本',
      time: '时间',
      preview: '预览',
      restore: '恢复',
      restoreConfirm: {
        title: '恢复历史版本',
        message: '确定要恢复到此版本吗？当前内容将被覆盖。',
        confirmText: '恢复',
      },
    },
    statusBar: {
      line: '行',
      column: '列',
      characters: '字符',
      words: '词',
      selected: '已选择',
    },
  },

  // 导出
  export: {
    title: '导出笔记',
    format: '导出格式',
    formatOptions: {
      markdown: 'Markdown (.md)',
      html: 'HTML (.html)',
      pdf: 'PDF (.pdf)',
      txt: '纯文本 (.txt)',
    },
    includeOptions: '包含选项',
    includeMeta: '包含元数据（标题、时间等）',
    includeTags: '包含标签',
    filename: '文件名',
    filenamePlaceholder: '请输入文件名',
    exportButton: '导出',
    exporting: '正在导出...',
    success: '导出成功',
    error: '导出失败',
    wordDoc: 'Word 文档',
    evernote: '印象笔记',
    untitledNote: '未命名笔记',
    exportNote: '导出笔记',
    batchExport: '批量导出笔记',
    zipArchive: 'ZIP 压缩包',
    wordDocDesc: '导出为 .doc 格式，可用 Word 打开',
    markdownDesc: '导出为 .md 格式',
    evernoteDesc: '导出为 .enex 格式，可导入印象笔记',
    jsonDesc: '导出为 JSON 数据格式',
    pdfDesc: '通过打印对话框保存为 PDF',
    htmlDesc: '导出为独立 HTML 文件',
    xmlDesc: '导出为 XML 数据格式',
    noTitle: '无标题',
    createTime: '创建时间',
    updateTime: '更新时间',
    tags: '标签',
  },

  // 导入
  import: {
    title: '导入笔记',
    format: '导入格式',
    formatOptions: {
      markdown: 'Markdown (.md)',
      html: 'HTML (.html)',
      txt: '纯文本 (.txt)',
      zip: '批量导入 (.zip)',
    },
    selectFile: '选择文件',
    orDragAndDrop: '或拖拽文件到这里',
    supportedFormats: '支持的格式：.md, .html, .txt, .zip',
    selectNotebook: '选择目标笔记本',
    selectNotebookPlaceholder: '请选择笔记本',
    autoCreateTags: '自动创建不存在的标签',
    importButton: '导入',
    importing: '正在导入...',
    parsing: '正在解析',
    startImport: '开始导入',
    prevStep: '上一步',
    nextStep: '下一步',
    success: '导入成功',
    partialSuccess: '导入完成（部分失败）',
    successMessage: '成功导入 {count} 条笔记',
    error: '导入失败',
    errorFileFormat: '不支持的文件格式',
    errorFileContent: '文件内容解析失败',
    confirmOverwrite: {
      title: '文件已存在',
      message: '已存在同名笔记，是否覆盖？',
      confirmText: '覆盖',
      cancelText: '跳过',
    },
  },

  // 键盘快捷键
  shortcuts: {
    save: '保存笔记',
    newNote: '新建笔记',
    editNote: '编辑笔记',
    cancelEdit: '取消编辑',
    toggleSidebar: '切换侧边栏',
    commandPalette: '命令面板',
    lockApp: '锁定应用',
    switchLayout: '切换布局模式',
    helpManual: '帮助手册',
  },

  // 命令面板
  commandPalette: {
    placeholder: '输入命令...',
    noResults: '没有匹配的命令',
    categories: {
      notes: '笔记',
      view: '视图',
      app: '应用',
    },
    commands: {
      newNote: '新建笔记',
      saveNote: '保存笔记',
      editNote: '编辑笔记',
      deleteNote: '删除笔记',
      toggleSidebar: '切换侧边栏',
      toggleDarkMode: '切换深色模式',
      openSettings: '打开设置',
      openTrash: '打开回收站',
      openBackup: '数据备份',
      openTemplates: '模板管理',
      newFromTemplate: '从模板新建',
      saveAsTemplate: '存为模板',
      viewHelp: '查看帮助手册',
      checkForUpdates: '检查更新',
      printNote: '打印笔记',
    },
  },

  // 验证
  validation: {
    required: '此字段为必填项',
    invalidFormat: '格式不正确',
    tooLong: '内容过长',
    tooShort: '内容过短',
    invalidUrl: '无效的 URL',
  },

  // 日期时间
  datetime: {
    today: '今天',
    yesterday: '昨天',
    justNow: '刚刚',
    minutesAgo: '{n}分钟前',
    hoursAgo: '{n}小时前',
    daysAgo: '{n}天前',
    weeksAgo: '{n}周前',
    monthsAgo: '{n}月前',
    yearsAgo: '{n}年前',
  },

  // 通知
  notification: {
    success: '操作成功',
    error: '操作失败',
    warning: '警告',
    info: '提示',
    saved: '保存成功',
    deleted: '删除成功',
    updated: '更新成功',
    created: '创建成功',
    networkError: '网络错误，请稍后重试',
    serverError: '服务器错误',
    showDetail: '查看详情',
    hideDetail: '收起详情',
  },

  // 后端错误码
  errorCodes: {
    DATABASE_ERROR: '数据库操作失败，请稍后重试',
    NOT_FOUND: '资源不存在',
    VALIDATION_ERROR: '验证失败',
    BUSINESS_ERROR: '操作失败',
    CONFIG_ERROR: '配置错误',
    INTERNAL_ERROR: '系统内部错误，请稍后重试',
    NETWORK_ERROR: '网络连接失败，请检查您的网络',
    TIMEOUT_ERROR: '请求超时，请稍后重试',
    PERMISSION_DENIED: '没有权限执行此操作',
    UNKNOWN_ERROR: '操作失败，请稍后重试',
    // 编码化业务错误
    DB_NOT_CONNECTED: '数据库未连接，请先选择配置',
    DB_CONNECTION_FAILED: '数据库连接失败：{0}',
    DB_MIGRATION_FAILED: '数据库迁移失败：{0}',
    UNSUPPORTED_EXPORT_FORMAT: '不支持的导出格式',
    UNSUPPORTED_IMPORT_FORMAT: '不支持的导入格式',
    SYNC_LOG_NOT_FOUND: '同步日志不存在',
    JSON_SERIALIZE_FAILED: '数据序列化失败：{0}',
    FILE_WRITE_FAILED: '写入文件失败：{0}',
    CANNOT_LINK_SELF: '不能链接到自身',
    PROFILE_NOT_FOUND: 'Profile 不存在：{0}',
    NO_VALID_SQL_STATEMENTS: 'SQL 文件中没有找到有效的 INSERT 语句',
    SQL_EXEC_FAILED: '执行 SQL 失败：{0}',
    PASSWORD_HASH_FAILED: '密码哈希失败：{0}',
    PASSWORD_HASH_INVALID: '密码哈希格式无效：{0}',
    // Server 后端错误
    SERVER_FEATURE_NOT_SUPPORTED: '此功能在服务器后端模式下不可用',
    SERVER_AUTH_FAILED: '服务器认证失败，请检查凭据',
    SERVER_FORBIDDEN: '服务器拒绝访问',
    SERVER_NOT_FOUND: '服务器上未找到该资源',
    SERVER_RATE_LIMITED: '请求过于频繁，请稍后重试',
    SERVER_ERROR: '服务器错误：{0}',
    SERVER_UNEXPECTED: '服务器返回异常：{0}',
    SERVER_CONNECTION_FAILED: '无法连接到服务器：{0}',
    SERVER_CONFIG_MISSING: '服务器配置缺失',
    SERVER_TIMEOUT: '服务器请求超时',
    SERVER_REQUEST_FAILED: '服务器请求失败：{0}',
    SERVER_PARSE_ERROR: '服务器响应解析失败：{0}',
    SERVER_VALIDATION_ERROR: '服务器验证失败',
    NOT_SERVER_BACKEND: '当前 Profile 不是服务器后端',
  },

  // Composable 通知字符串
  composable: {
    loadingData: '正在加载',
    initFailed: '初始化失败，请刷新页面重试',
    savingNote: '正在保存笔记',
    noteSaved: '笔记保存成功',
    saveNoteFailed: '保存笔记失败',
    deletingNote: '正在删除笔记',
    noteDeleted: '笔记已删除',
    deleteNoteFailed: '删除笔记失败',
    loadingNotebooks: '正在加载笔记本',
    loadNotebooksFailed: '加载笔记本失败',
    savingNotebook: '正在保存笔记本',
    notebookSaved: '笔记本保存成功',
    saveNotebookFailed: '保存笔记本失败',
    deletingNotebook: '正在删除笔记本',
    notebookDeleted: '笔记本已删除',
    deleteNotebookFailed: '删除笔记本失败',
    loadingTags: '正在加载标签',
    loadTagsFailed: '加载标签失败',
    savingTag: '正在保存标签',
    tagSaved: '标签保存成功',
    saveTagFailed: '保存标签失败',
    deletingTag: '正在删除标签',
    tagDeleted: '标签已删除',
    deleteTagFailed: '删除标签失败',
    loadingNotes: '正在加载笔记',
    loadNotesFailed: '加载笔记失败',
    countingNotes: '正在统计笔记',
    countNotesFailed: '统计笔记失败',
    loadHistoryFailed: '加载历史记录失败',
  },

  // 校验错误信息
  validationError: {
    notebookNameRequired: '笔记本名称不能为空',
    notebookNameTooLong: '笔记本名称不能超过{max}个字符',
    tagNameRequired: '标签名称不能为空',
    tagNameTooLong: '标签名称不能超过{max}个字符',
    noteTitleRequired: '笔记标题不能为空',
    noteTitleTooLong: '笔记标题不能超过{max}个字符',
    urlRequired: 'URL 不能为空',
    urlInvalid: '请输入有效的 URL',
    noteContentTooLarge: '笔记内容超过大小限制（最大 {size}MB）',
  },

  // 无障碍标签
  aria: {
    sidebar: '侧边栏导航',
    noteList: '笔记列表',
    noteEditor: '笔记编辑器',
    toolbar: '编辑工具栏',
    closeDialog: '关闭对话框',
    tocNavigation: '目录导航',
    searchInput: '搜索输入',
    noteItem: '笔记项',
    notebookItem: '笔记本项',
    tagItem: '标签项',
    prevPage: '上一页',
    nextPage: '下一页',
    pageInfo: '{current} / {total}',
    selectOption: '选择选项',
    expandDropdown: '展开下拉菜单',
  },

  // 目录组件
  toc: {
    title: '目录',
    untitledHeading: '(无标题)',
  },

  // 分页组件
  pagination: {
    total: '共 {total} 条',
    itemsPerPage: '{size} 条/页',
    prevPage: '上一页',
    nextPage: '下一页',
  },

  // 历史记录对话框
  history: {
    title: '历史记录',
    notebookName: '笔记本名称',
    noteTitle: '标题',
    contentType: '内容类型',
    tags: '标签',
    operateType: '操作类型',
    operateTime: '操作时间',
    action: '操作',
    empty: '暂无历史记录',
    view: '查看',
    contentView: '内容查看',
    oldContent: '旧内容',
    newContent: '新内容',
    richText: '富文本',
    operateCreate: '添加',
    operateUpdate: '修改',
    operateDelete: '删除',
    operateSource: '操作来源',
    sourceUser: '用户操作',
    sourceMcp: 'AI 工具',
    sideBySideView: '并排查看',
    diffView: '差异对比',
  },

  // 编辑器设置对话框
  editorSettings: {
    title: '设置',
    notebook: '笔记本',
    selectNotebook: '请选择笔记本',
    tags: '标签',
    ariaLabel: '笔记设置',
  },

  // 印象笔记导入
  importEvernote: {
    untitledNote: '未命名笔记',
    parseNoteFailed: '解析笔记失败',
    enexFormatError: 'ENEX 文件格式错误',
    enexNoNotes: 'ENEX 文件中没有找到笔记',
    untitled: '未命名',
    parseNoteIndexFailed: '解析第 {index} 个笔记失败',
    unknownError: '未知错误',
  },

  // 颜色选择器
  colorPicker: {
    selectColor: '选择颜色',
  },

  // 样式选择器
  stylePicker: {
    presetColors: '预设颜色',
    shades: '色阶',
    customStyle: '自定义样式',
    inputPlaceholder: '输入 Tailwind 类名，如 text-red-500',
    apply: '应用',
    preview: '预览:',
    current: '当前:',
    selectStyle: '选择样式',
  },

  // 图标选择器
  iconPicker: {
    searchPlaceholder: '搜索图标...',
    noMatch: '未找到匹配的图标',
    selected: '已选:',
    selectIcon: '选择图标',
    clearSearch: '清除搜索',
  },

  // 导入来源
  importSource: {
    evernoteName: '印象笔记',
    evernoteDescription: '导入 .enex 格式的印象笔记导出文件',
    youdaoName: '有道笔记',
    youdaoDescription: '导入有道笔记导出的 ZIP 压缩包',
    notionName: 'Notion',
    notionDescription: '导入 Notion 导出的 Markdown ZIP 压缩包',
  },

  // 字体名称
  fontName: {
    microsoftYaHei: '微软雅黑',
    pingFang: '苹方',
    simSun: '宋体',
    kaiTi: '楷体',
    fangSong: '仿宋',
  },

  // Notion 导入
  importNotion: {
    untitledNote: '未命名笔记',
    noNotesFound: 'ZIP 文件中没有找到 Notion 笔记文件 (.md)',
    parseFailed: '解析 {filepath} 失败: {error}',
    unknownError: '未知错误',
  },

  // 设置
  settings: {
    title: '设置',
    general: '通用',
    appearance: '外观',
    language: '语言',
    languageZh: '中文',
    languageEn: 'English',
    theme: '主题',
    themeLight: '浅色',
    themeDark: '深色',
    themeSystem: '跟随系统',
    layoutMode: '布局模式',
    layoutAuto: '自动',
    layoutDesktop: '桌面',
    layoutTablet: '平板',
    layoutMobile: '手机',
    shortcutsTitle: '快捷键',
    shortcutsSystem: '系统',
    shortcutsReset: '重置',
    shortcutsResetAll: '全部重置为默认',
    shortcutsRecording: '请按下快捷键...',
    shortcutsConflict: '与「{name}」冲突',
    shortcutsResetAllConfirm: '确定重置所有快捷键为默认值？',
    shortcutsResetAllDone: '已重置所有快捷键',
    editorFontSize: '编辑器字体大小',
    autoSave: '自动保存',
    autoSaveEnabled: '启用自动保存',
    saved: '设置已保存',
    saveFailed: '保存设置失败',
    autoBackup: '自动备份',
    autoBackupEnabled: '启用自动备份',
    autoBackupInterval: '备份间隔',
    autoBackupRetention: '保留份数',
    autoBackupIntervalHours: '{n} 小时',
    autoBackupRetentionCount: '最近 {n} 份',
    backupNow: '立即备份',
    backupSuccess: '备份成功：{name}',
    backupFailed: '备份失败',
    autoBackupFailed: '自动备份失败',
    lastBackup: '上次备份',
    never: '从未备份',
    // 云备份
    cloudBackup: '云备份',
    cloudBackupEnabled: '启用云备份',
    cloudBackupProvider: '存储提供商',
    cloudBackupEndpoint: '服务端点',
    cloudBackupBucket: '存储桶',
    cloudBackupRegion: '区域',
    cloudBackupAccessKeyId: 'Access Key ID',
    cloudBackupSecretAccessKey: 'Secret Access Key',
    cloudBackupPrefix: '目录前缀',
    cloudBackupUsername: '用户名',
    cloudBackupPassword: '密码',
    cloudBackupRetention: '云端保留份数',
    cloudBackupTestConnection: '测试连接',
    cloudBackupTestSuccess: '连接成功',
    cloudBackupTestFailed: '连接失败',
    cloudBackupNow: '备份到云端',
    cloudBackupSuccess: '云备份成功：{name}',
    cloudBackupFailed: '云备份失败',
    cloudBackupUploadFailed: '上传云端失败',
    cloudBackupLastBackup: '最近云端备份',
    cloudBackupProviderS3: 'AWS S3',
    cloudBackupProviderOss: '阿里云 OSS',
    cloudBackupProviderCos: '腾讯云 COS',
    cloudBackupProviderMinio: 'MinIO',
    cloudBackupProviderWebdav: 'WebDAV',
    cloudBackupConfigSaved: '云备份配置已保存',
    cloudBackupConfigSaveFailed: '保存云备份配置失败',
    cloudBackupDownloadSuccess: '已下载到本地：{name}',
    cloudBackupDownloadFailed: '下载失败',
    cloudBackupListTitle: '云端备份列表',
    cloudBackupEmpty: '暂无云端备份',
    cloudBackupDownload: '下载',
    cloudBackupTesting: '测试中...',
    // 安全设置
    security: '安全',
    lockMode: '锁屏方式',
    lockModeNone: '关闭',
    lockModePassword: '密码',
    lockModeBiometric: '生物识别',
    lockTimeout: '自动锁定',
    lockTimeoutNone: '仅启动时',
    lockTimeoutMinutes: '{n} 分钟无操作',
    lockOnMinimize: '最小化时锁定',
    // 屏幕保护
    screenSaver: '屏幕保护',
    screenSaverEnabled: '启用屏保',
    screenSaverEnabledDesc: '定时强制休息，保护视力',
    screenSaverIdleTime: '空闲触发时间',
    screenSaverDuration: '屏保持续时长',
    screenSaverDurationUnlimited: '无限',
    screenSaverMinutes: '{n} 分钟',
    screenSaverBgColor: '屏保底色',
    screenSaverBgImage: '背景图片',
    screenSaverText: '显示文字',
    screenSaverTextColor: '文字颜色',
    screenSaverFontSize: '文字字号',
    screenSaverSelectImage: '选择图片',
    screenSaverClearImage: '清除图片',
    screenSaverImageSet: '背景图片已设置',
    screenSaverImageFailed: '设置背景图片失败',
    setPassword: '设置密码',
    changePassword: '修改密码',
    removePassword: '移除密码',
    passwordSet: '密码已设置',
    passwordRemoved: '密码已移除',
    passwordChanged: '密码已修改',
    currentPassword: '当前密码',
    newPassword: '新密码',
    confirmPassword: '确认密码',
    passwordMismatch: '两次输入的密码不一致',
    passwordTooShort: '密码至少 4 位',
    passwordIncorrect: '密码错误',
    biometricNotSupported: '当前设备不支持生物识别',
    // MCP 设置
    mcp: 'MCP (AI 工具集成)',
    mcpEnabled: '启用 MCP',
    mcpEnabledDesc: '允许 AI 工具通过 MCP 协议操作笔记',
    mcpToolSearch: '搜索笔记',
    mcpToolGetNote: '获取笔记详情',
    mcpToolCreateNote: '创建笔记',
    mcpToolUpdateNote: '更新笔记',
    mcpToolDeleteNote: '删除笔记',
    mcpToolListNotebooks: '列出笔记本',
    mcpToolCreateNotebook: '创建笔记本',
    mcpToolListTags: '列出标签',
    mcpToolCreateTag: '创建标签',
    mcpToolNoteStats: '笔记统计',
    mcpToolSearchTodos: '搜索待办',
    mcpToolGetTodo: '获取待办详情',
    mcpToolCreateTodo: '创建待办',
    mcpToolUpdateTodo: '更新待办',
    mcpToolCompleteTodo: '切换完成状态',
    mcpToolDeleteTodo: '删除待办',
    mcpToolListTodoLists: '列出待办清单',
    mcpToolCreateTodoList: '创建待办清单',
    mcpToolTodoStats: '待办统计',
    // 系统维护
    maintenance: '系统维护',
    syncDesc: '将当前 Profile 的数据同步到其他 Profile',
    syncHistoryDesc: '查看和管理历史同步记录',
    // MCP 访问控制
    mcpAccess: 'AI 访问权限',
    mcpAccessInherit: '继承',
    mcpAccessReadWrite: '读写',
    mcpAccessReadOnly: '只读',
    mcpAccessDeny: '禁止',
    mcpAccessDesc: '控制 AI 通过 MCP 对此数据的访问权限',
  },

  // 锁屏
  lockScreen: {
    title: '应用已锁定',
    subtitle: '请输入密码解锁',
    placeholder: '输入密码...',
    unlock: '解锁',
    incorrectPassword: '密码错误，请重试',
    useBiometric: '使用生物识别',
  },

  // 屏保
  screenSaver: {
    remaining: '剩余时间',
    exit: '退出',
    tipExit: '按 ESC 或点击任意位置可提前结束',
    defaultText: '请休息一下眼睛',
  },

  // 回收站
  trash: {
    title: '回收站',
    empty: '回收站为空',
    emptyTrash: '清空回收站',
    restore: '恢复',
    permanentDelete: '永久删除',
    emptyConfirm: {
      title: '清空回收站',
      message: '确定要清空回收站吗？所有已删除的笔记将被永久删除，此操作不可恢复。',
      confirmText: '清空',
    },
    deleteConfirm: {
      title: '永久删除',
      message: '确定要永久删除这条笔记吗？此操作不可恢复。',
      confirmText: '永久删除',
    },
    restored: '笔记已恢复',
    restoreFailed: '恢复笔记失败',
    deleted: '笔记已永久删除',
    deleteFailed: '永久删除失败',
    emptied: '回收站已清空',
    emptyFailed: '清空回收站失败',
  },

  // 笔记置顶
  pin: {
    pin: '置顶',
    unpin: '取消置顶',
    pinned: '已置顶',
  },

  // 笔记收藏/星标
  star: {
    starred: '收藏',
    toggleStar: '收藏/取消收藏',
    star: '收藏',
    unstar: '取消收藏',
  },

  // 模板
  template: {
    title: '模板管理',
    name: '模板名称',
    create: '新建模板',
    edit: '编辑模板',
    delete: '删除模板',
    deleteConfirm: '确定要删除模板「{name}」吗？',
    useTemplate: '使用模板',
    noTemplates: '暂无模板',
    saveAsTemplate: '存为模板',
    saveAsTemplateSuccess: '已保存为模板',
    namePlaceholder: '请输入模板名称',
    back: '返回',
    editorPlaceholder: '在此编辑模板内容...',
    saveSuccess: '模板保存成功',
  },

  // 附件
  attachment: {
    title: '附件',
    add: '添加附件',
    delete: '删除附件',
    deleteConfirm: '确定要删除此附件吗？',
    open: '打开',
    empty: '暂无附件',
    fileSizeKB: '{size} KB',
    fileSizeMB: '{size} MB',
    uploadFailed: '附件上传失败',
    fileTooLarge: '文件大小超过 50 MB 限制',
    count: '{count} 个附件',
  },

  // 待办
  todo: {
    title: '待办',
    add: '新建待办',
    addPlaceholder: '输入待办标题…',
    edit: '编辑待办',
    delete: '删除待办',
    deleteConfirm: '确定要删除此待办吗？',
    empty: '暂无待办',
    emptyCompleted: '暂无已完成的待办',
    titleRequired: '待办标题不能为空',
    completed: '已完成',
    dueDate: '截止日期',
    noDueDate: '无截止日期',
    dueToday: '今天到期',
    overdue: '已过期',
    priority: '优先级',
    priorityNone: '无',
    priorityLow: '低',
    priorityMedium: '中',
    priorityHigh: '高',
    viewAll: '全部',
    viewToday: '今天',
    viewPlanned: '计划',
    viewOverdue: '已过期',
    viewTrash: '回收站',
    listTitle: '清单',
    newList: '新建清单',
    listName: '清单名称',
    deleteList: '删除清单',
    deleteListConfirm: '确定要删除此清单吗？清单下的待办不会被删除。',
    unassigned: '未归类',
    restore: '恢复',
    permanentDelete: '彻底删除',
    emptyTrash: '清空回收站',
    emptyTrashConfirm: '确定要清空回收站吗？此操作不可恢复。',
    searchPlaceholder: '搜索待办…',
    showCompleted: '显示已完成',
    description: '备注',
    createFailed: '创建待办失败',
    updateFailed: '更新待办失败',
    remindAt: '提醒时间',
    recurrence: '重复',
    recurrenceNone: '不重复',
    recurrenceDaily: '每天',
    recurrenceWeekly: '每周',
    recurrenceMonthly: '每月',
    recurrenceYearly: '每年',
    recurrenceInterval: '每 N 个周期',
    recurrenceEndDate: '重复截止日',
    subtask: '子任务',
    addSubtask: '添加子任务',
    subtaskCount: '{count} 个子任务',
    relatedNote: '关联笔记',
    noRelatedNote: '不关联',
    tags: '标签',
    batchMode: '批量',
    selectAll: '全选',
    selectedCount: '已选 {count} 项',
    batchComplete: '标记完成',
    batchDelete: '删除',
    batchMove: '移动到清单',
    exitBatch: '退出批量',
    quadrant: '四象限',
    q1: '重要且紧急',
    q2: '重要不紧急',
    q3: '紧急不重要',
    q4: '不重要不紧急',
    statistics: '统计',
    totalCount: '总数',
    completedCount: '已完成',
    pendingCount: '未完成',
    todayCount: '今日到期',
    overdueCount: '已过期',
    completionRate: '完成率',
    pomodoro: '番茄钟',
    pomodoroStart: '开始专注',
    pomodoroPause: '暂停',
    pomodoroReset: '重置',
    pomodoroDone: '专注时间结束，休息一下',
    noSelection: '从左侧选择一个待办查看详情',
  },

  // 多窗口
  multiWindow: {
    openInNewWindow: '在新窗口中打开',
  },

  // 加密
  encryption: {
    encrypt: '加密笔记',
    decrypt: '解密笔记',
    enterPassword: '请输入密码',
    confirmPassword: '确认密码',
    passwordMismatch: '两次输入的密码不一致',
    passwordRequired: '请输入密码',
    encryptSuccess: '笔记已加密',
    decryptSuccess: '笔记已解密',
    encryptFailed: '加密失败',
    decryptFailed: '解密失败，请检查密码是否正确',
    isEncrypted: '此笔记已加密',
    encryptedPlaceholder: '此笔记内容已加密，请点击解密按钮输入密码查看。',
  },

  // 笔记链接
  noteLink: {
    title: '关联笔记',
    addLink: '添加链接',
    searchPlaceholder: '搜索笔记...',
    noLinks: '暂无关联笔记',
    noResults: '未找到匹配的笔记',
    removeLink: '移除链接',
  },

  // Profile / Setup
  profile: {
    selectTitle: '选择笔记库',
    newProfile: '新建配置',
    editProfile: '编辑配置',
    deleteProfile: '删除配置',
    deleteConfirm: '确定要删除配置「{name}」吗？关联的加密密钥也将被删除。',
    autoConnect: '下次自动连接上次使用的配置',
    lastUsed: '上次使用',
    connect: '连接',
    connecting: '连接中...',
    switchProfile: '切换配置',
    manageProfiles: '管理配置',
    currentProfile: '当前配置',
    noProfiles: '暂无配置，请创建新配置',
  },

  setup: {
    title: '初始化设置',
    editTitle: '编辑数据库配置',
    subtitle: '配置您的笔记数据库连接',
    editSubtitle: '修改现有配置的数据库连接设置',
    step: {
      dbType: '数据库类型',
      connection: '连接配置',
      security: '安全设置',
      finish: '完成',
    },
    dbType: {
      sqlite: 'SQLite',
      sqliteDesc: '本地文件数据库，无需安装，开箱即用',
      mysql: 'MySQL',
      mysqlDesc: '适合团队协作或远程访问',
      postgres: 'PostgreSQL',
      postgresDesc: '功能丰富的企业级数据库',
      enoteServer: 'ENote Server',
      enoteServerDesc: '连接远程 ENote 兼容 API 服务器',
    },
    // SQLite
    dbPath: '数据库路径',
    dbPathPlaceholder: '选择数据库存储位置',
    selectPath: '选择',
    createNewDb: '新建',
    openExistingDb: '打开已有',
    defaultPath: '默认位置（应用数据目录）',
    // MySQL / PostgreSQL
    host: '主机地址',
    hostPlaceholder: '如 localhost 或 192.168.1.100',
    port: '端口',
    database: '数据库名',
    databasePlaceholder: '如 enote',
    authMethod: '认证方式',
    authPassword: '密码登录',
    authCertificate: '证书登录',
    username: '用户名',
    usernamePlaceholder: '数据库用户名',
    password: '密码',
    passwordPlaceholder: '数据库密码',
    // SSL
    sslMode: 'SSL 模式',
    sslModeDisabled: '关闭',
    sslModePreferred: '优先',
    sslModeRequired: '必须',
    sslModeVerifyCa: '验证 CA',
    sslModeVerifyIdentity: '验证身份',
    sslModeVerifyFull: '完全验证',
    caCert: 'CA 证书',
    clientCert: '客户端证书',
    clientKey: '客户端私钥',
    selectFile: '选择文件',
    // Security
    contentEncryption: '内容加密',
    contentEncryptionDesc: '对笔记内容进行 AES-256 加密，密钥安全存储在系统钥匙串中',
    contentEncryptionWarning: '启用加密后，标题仍可搜索，但内容将无法通过全文搜索检索',
    generateKey: '自动生成密钥',
    keyGenerated: '密钥已生成并将存储到系统钥匙串',
    // Profile name
    profileName: '配置名称',
    profileNamePlaceholder: '如「本地笔记」或「工作数据库」',
    defaultName: {
      sqlite: '本地笔记',
      mysql: 'MySQL 数据库',
      postgres: 'PostgreSQL 数据库',
    },
    profileIcon: '图标',
    // Server
    serverUrl: '服务器地址',
    serverUrlPlaceholder: '如 https://notes.example.com',
    serverAuthMethod: '认证方式',
    serverAuthNone: '无认证',
    serverAuthBearer: 'Bearer Token',
    serverAuthBasic: 'Basic（用户名/密码）',
    serverAuthJwt: 'JWT（自动刷新）',
    serverAuthCustomHeader: '自定义 Header',
    serverAuthOAuth2: 'OAuth 2.0',
    serverToken: 'Token',
    serverTokenPlaceholder: '输入 Bearer Token 或 API Key',
    serverRefreshUrl: 'Token 刷新地址',
    serverRefreshUrlPlaceholder: '如 /auth/refresh',
    serverHeaderName: 'Header 名称',
    serverHeaderNamePlaceholder: '如 X-API-Key',
    serverHeaderValue: 'Header 值',
    serverOAuthTokenUrl: 'Token URL',
    serverOAuthClientId: 'Client ID',
    serverOAuthClientSecret: 'Client Secret',
    serverOAuthScopes: '权限范围',
    serverEncryptionNote: '内容加密由服务端处理，无需在客户端配置',
    serverTimeout: '请求超时（秒）',
    // Actions
    resetDefaults: '恢复默认值',
    testConnection: '测试连接',
    testSuccess: '连接成功',
    testFailed: '连接失败',
    saveAndConnect: '保存并连接',
    saving: '保存中...',
    prev: '上一步',
    next: '下一步',
  },

  // 跨 Profile 同步
  sync: {
    title: '跨 Profile 同步',
    source: '源',
    target: '目标',
    selectTarget: '选择目标 Profile',
    mode: '同步模式',
    modeAppend: '追加',
    modeAppendDesc: '追加到目标，不影响已有数据',
    modeOverwrite: '覆盖',
    modeOverwriteDesc: '清空目标后全量写入',
    scope: '同步范围',
    scopeNotebooks: '笔记本',
    scopeTags: '标签',
    scopeNotes: '笔记',
    scopeNoteHistories: '笔记历史',
    scopeTemplates: '模板',
    scopeTodos: '待办',
    scopeTodoLists: '待办清单',
    scopeSettings: '设置',
    backup: '备份设置',
    autoBackup: '同步前自动备份',
    backupFormat: '备份格式',
    preview: '预览',
    startSync: '开始同步',
    syncing: '正在同步...',
    backupSource: '备份源数据库',
    backupTarget: '备份目标数据库',
    clearTarget: '清空目标数据',
    completed: '同步完成',
    failed: '同步失败',
    summary: '汇总',
    table: '表',
    sourceCount: '源端',
    successCount: '成功',
    failedCount: '失败',
    total: '合计',
    details: '明细',
    viewDetails: '查看详情',
    exportLog: '导出日志',
    noProfiles: '没有其他可用的 Profile',
    confirmOverwrite: '覆盖模式将清空目标数据库的所有数据，是否继续？',
    targetPassword: '目标数据库密码',
    targetPasswordHint: '如果目标 Profile 使用密码认证，请输入密码',
    // 同步记录管理
    history: '同步记录',
    historyTitle: '同步记录管理',
    noHistory: '暂无同步记录',
    deleteLog: '删除记录',
    clearAll: '清空所有记录',
    confirmDelete: '确定删除此同步记录？',
    confirmClearAll: '确定清空所有同步记录？',
    filterTable: '按表筛选',
    filterStatus: '按状态筛选',
    allTables: '所有表',
    allStatus: '所有状态',
    statusSuccess: '成功',
    statusFailed: '失败',
    statusSkipped: '跳过',
    duration: '耗时',
    backupFiles: '备份文件',
    collapseDetails: '收起明细',
    syncCompleteWithFailures: '同步完成: {success} 成功, {failed} 失败',
    syncCompleteSuccess: '同步完成: {count} 条记录',
    deleteSuccess: '删除成功',
    clearAllSuccess: '已清空所有同步记录',
    exportSuccess: '导出成功',
    settingsCount: '{count} 项设置',
    sourceNoteNotFound: '源端笔记未找到',
    progressClearTarget: '清空目标数据',
    progressSettings: '设置',
  },

  // 颜色
  colors: {
    red: '红色',
    orange: '橙色',
    amber: '琥珀色',
    yellow: '黄色',
    lime: '青柠色',
    green: '绿色',
    emerald: '翠绿色',
    teal: '青色',
    cyan: '青蓝色',
    sky: '天蓝色',
    blue: '蓝色',
    indigo: '靛蓝色',
    violet: '紫罗兰',
    purple: '紫色',
    fuchsia: '品红色',
    pink: '粉色',
    rose: '玫瑰色',
    slate: '石板灰',
    gray: '灰色',
    zinc: '锌灰色',
  },

  // 图标分类
  iconCategory: {
    all: '全部',
    file: '文件',
    folder: '文件夹',
    book: '书籍',
    tag: '标签',
    edit: '编辑',
    list: '列表',
    time: '时间',
    media: '媒体',
    shape: '形状',
    user: '用户',
    other: '其他',
  },

  // 启动错误
  error: {
    startupError: '应用启动失败',
    startupErrorMessage: '无法连接到数据库或加载配置，请检查后重试。',
  },

  // 帮助手册
  help: {
    title: '帮助手册',
    description: '查看应用使用文档',
    viewButton: '查看',
    loading: '正在加载手册...',
    loadFailed: '手册加载失败',
    searchPlaceholder: '搜索手册内容...',
    noResults: '没有匹配的内容',
    openInNewWindow: '在新窗口中打开',
    backToTop: '返回顶部',
  },

  // 日志管理
  log: {
    title: '日志管理',
    description: '查看和管理应用日志',
    dbLogs: '操作日志',
    fileLogs: '系统日志',
    allLevels: '全部级别',
    allModules: '全部模块',
    searchPlaceholder: '搜索日志内容...',
    time: '时间',
    level: '级别',
    module: '模块',
    action: '操作',
    message: '描述',
    noLogs: '暂无日志记录',
    clearAll: '清空全部',
    cleanupOld: '清理30天前',
    clearedCount: '已清理 {count} 条记录',
    noFiles: '暂无日志文件',
    searchInFile: '搜索文件内容...',
    selectFileHint: '请选择左侧日志文件查看',
    fileDeleted: '日志文件已删除',
    cleanupOldFiles: '清理30天前文件',
    refresh: '刷新',
    openLogManager: '打开日志管理',
    fileLogEnabled: '启用文件日志',
    fileLogLevel: '文件日志级别',
    fileRetentionDays: '文件保留天数',
    dbLogEnabled: '启用操作日志',
    dbRetentionDays: '操作日志保留天数',
    frontendLogLevel: '前端日志级别',
  },

  // 有道笔记导入
  importYoudao: {
    untitledNote: '未命名笔记',
    noNotesFound: 'ZIP 文件中没有找到笔记文件 (.html, .md)',
    parseFailed: '解析 {filepath} 失败: {error}',
    unknownError: '未知错误',
  },

  // 自动更新
  updater: {
    checking: '正在检查更新...',
    newVersion: '发现新版本',
    newVersionAvailable: '新版本 {version} 已发布',
    releaseNotes: '更新说明',
    downloading: '正在下载更新...',
    downloadProgress: '下载进度: {percent}%',
    installing: '正在安装更新...',
    updateNow: '立即更新',
    later: '稍后提醒',
    upToDate: '当前已是最新版本',
    checkFailed: '检查更新失败',
    downloadFailed: '下载更新失败',
    restartRequired: '更新已下载，重启应用以完成安装',
    restartNow: '立即重启',
    currentVersion: '当前版本: {version}',
    checkForUpdates: '检查更新',
  },
} as const
