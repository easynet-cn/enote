...
import type { ShowNotebook, ShowTag, ShowNote } from '../types'
...
const emit = defineEmits<{
  submit: [notebookId: string, tagIds: string[]]
}>()
...
// 监听activeNote变化，同步表单数据
watch(
  () => props.activeNote,
  (newNote) => {
    if (newNote) {
      form.notebookId = newNote.notebookId ?? ''
      form.tagIds = newNote.tags?.map((t) => t.id) ?? []
    }
  },
  { immediate: true },
)
...
watch(visible, (newVisible) => {
  if (newVisible && props.activeNote) {
    form.notebookId = props.activeNote.notebookId ?? ''
    form.tagIds = props.activeNote.tags?.map((t) => t.id) ?? []
  }
})
...
const handleSubmit = () => {
  emit('submit', form.notebookId, form.tagIds)
  visible.value = false
}
</script>
