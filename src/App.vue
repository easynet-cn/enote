<template>
  <el-container class="flex h-screen bg-white">
    ...
    <!-- 笔记列表组件 -->
    <NoteList :notebooks="notebooks" :notes="notes" :active-notebook="state.activeNotebook"
      :active-note="state.activeNote" v-model:current-page="state.notePageIndex" v-model:page-size="state.notePageSize"
      v-model:total="state.noteTotal" v-model:query="query" @set-active-note="setActiveNote"
      @update-search-query="handleUpdateSearchQuery" @size-change="handleNoteSizeChange"
    ...
    <Editor v-model:history-data="histories" v-model:current-page="state.historyPageIndex"
      ...
      @size-change="handleNoteHistorySizeChange" @current-change="handleNoteHistoryCurrentChange" />
  </el-container>
</template>
