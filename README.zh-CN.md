...
- **代码块语言选择** - 代码块快速选择编程语言，支持 25+ 种语言语法高亮
- **目录大纲** - 自动生成文档目录结构
- **拖拽排版** - 拖拽手柄调整内容块顺序
- **智能粘贴** - 上下文感知的粘贴格式处理
- **图片懒加载** - 图片延迟加载优化性能
- **打印支持** - 通过系统打印对话框直接打印笔记
- **编辑器字体大小** - 设置中可配置编辑器字体大小（12-20px）
...
- **命令面板** - Ctrl+P 快速执行操作
- **自定义快捷键** - 可配置的键盘快捷键
- **笔记排序** - 支持按标题、创建时间、更新时间排序（升序/降序）
- **批量操作** - 多选笔记批量移动、批量删除
- **笔记收藏** - 星标/收藏笔记，快速标识重要内容
- **文件附件** - 为笔记添加文件附件（PDF、文档等），本地存储管理
- **笔记本层级** - 嵌套笔记本树结构，支持展开/折叠
- **配置编辑** - 支持编辑已有数据库配置
